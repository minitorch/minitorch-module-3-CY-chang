...
class Exp(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        # TODO: Implement for Task 2.4.
        (t1, ) = ctx.saved_values
        back_values = grad_output.f.exp_map(t1)
        return grad_output.f.mul_zip(back_values, grad_output)
...
class Permute(Function):
    ...
    def forward(ctx: Context, a: Tensor, order: Tensor) -> Tensor:
        ...
        order_list = list(order.to_numpy().flatten().astype(int))
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        (a, order_list) = ctx.saved_values
        new_order = [0] * len(order_list)
        for i, idx in enumerate(order_list):
            new_order[idx] = i
        backward_tensor_data = grad_output._tensor.permute(*new_order)
        return grad_output._new(backward_tensor_data), 0
